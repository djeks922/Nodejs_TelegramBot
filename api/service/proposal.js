...
export const getProposals = async (filter = {}) => {
  try {
    const proposal = await Proposal.find(filter)
      ...
      .populate("acceptedBy", { chatID: 1, username: 1, name: 1 })
  ...
  try {
    const proposal = lean
      ...
      ? await Proposal.findOne({ _id: id })
          ...
          .populate("acceptedBy", { chatID: 1, username: 1, name: 1 })
          .populate("rejectedFor", { name: 1 })
      : await Proposal.findOne({ _id: id }).populate("approvedBy", {
          username: 1,
        });
...
export const getProposalByKeyword = async (name) => {
  try {
    const proposal = await Proposal.findOne({
      $or: [
        { name: { $regex: name, $options: "gi" } },
        { website: { $regex: name, $options: "gi" } },
        { twitter: { $regex: name, $options: "gi" } },
        { telegram: { $regex: name, $options: "gi" } },
        { developerUsername: { $regex: name, $options: "gi" } },
      ],
    });
