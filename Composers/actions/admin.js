import { deleteProposalByID, updateProposalByID } from "../../api/service/proposal.js";
...
const approveButtons = () => {
  return Markup.inlineKeyboard([Markup.button.callback('sure', `app Sure`),Markup.button.callback('cancel',`app Cancel`)]) 
}
export const approveProposal = async (ctx, proposal) => {
  try {
    if(proposal.status === 'rejected-by-admin'){
      ...
      return await ctx.reply(`Proposal rejected,Are you sure to approve it?`,approveButtons())
    }
...
export const approveProposal_rejectCase = async(ctx) => {
  try {
    ...
    if(data === 'Sure') {
      await updateProposalByID(ctx.session.currentRejectedProposal._id, {status: 'approved', approvedBy: ctx.session.consumer._id})
      await ctx.answerCbQuery('Approved!')
...
export const approveIndividual = async (ctx, proposal, approvedForID) => {
  try {
    ...
    if ( index === -1) {
      ...
      await ctx.answerCbQuery(
        `Approved for Influencer: @${proposal.approvedFor[proposal.approvedFor.length-1].username}`
      );
    } else {
      await proposal.populate('approvedFor')
      await ctx.answerCbQuery(
...
const rejectButtons = () => {
  return Markup.inlineKeyboard([Markup.button.callback('sure', `rej Sure`),Markup.button.callback('cancel',`rej Cancel`)])
}
...
export const rejectAdminProposal = async (ctx, proposal) => {
  try {
    if(proposal.approvedBy) {
      ctx.session.currentRejectedProposal = proposal
      await ctx.answerCbQuery()
      return await ctx.reply(`Proposal approved by ${proposal.approvedBy.username},Are you sure to reject it?`,rejectButtons())
    }
    if(proposal.status === 'rejected-by-admin')
      return await ctx.answerCbQuery("Proposal already rejected!");

    const res = await updateProposalByID(proposal._id, {status: 'rejected-by-admin'});
...
export const rejectAdminProposal_approvedCase = async (ctx) => {
  try {
    ...
    if(data === 'Sure') {
      await updateProposalByID(ctx.session.currentRejectedProposal._id, {status: 'rejected-by-admin', approvedBy: null})
...
export const rejectIndividual = async(ctx, proposal, rejectForID) => {
  try {
    ...
    await ctx.telegram.sendMessage(proposal.consumer.chatID, 'Your proposal for ')
  } catch (error) {
