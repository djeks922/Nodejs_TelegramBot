...
paymentConsumerScene.enter(async( ctx) => {
    console.log(ctx)
    await ctx.reply('Please enter TaxID of transaction for related package(influencer)')
...
paymentConsumerScene.on('text', async(ctx) => {
    try {
        ...
        await ctx.telegram.sendMessage(admin.chatID, consumerPaymentText(ctx,proposal,pkg),adminPaymentButtons(tr._id,proposal._id))
