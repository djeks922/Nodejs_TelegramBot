...
import {createTransaction,getTransactionByID,getVerifiedTransactions} from '../../../api/service/transaction.js'
...
txIDSubmition.on('text', async (ctx)=> {
    console.log(ctx.scene.state.transaction)
    try {
        const transactionI = {
        ...
        await ctx.telegram.sendMessage(ctx.scene.state.transaction.FOR.influencer.chatID, adminPaymentText(transactionI),influencerPaymentButtons(tr._id))
...
const paymentAdminScene = new WizardScene('payment-scene-toInfluencer-id',transactionSelection,txIDSubmition)
...
paymentAdminScene.enter(async( ctx) => {
    try {
        ...
        for(let [i,transaction] of transactions.entries()){
          text = text.concat(`${i}. For token ${transaction.proposal.name}(${transaction.from.name}) and package ${transaction.FOR.package.name}\n`)
        }
...
// paymentAdminScene.on('message', async(ctx) => {
//     try {
